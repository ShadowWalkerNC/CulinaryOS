...
# ---------------------------------------------------------------------------
# Tool 8: Post-Pilot Loyalty Marketing / Postcard Dispatch
# ---------------------------------------------------------------------------
@mcp.tool()
def send_marketing_postcard(customer_name: str, address: str, discount_percent: float, coupon_message: str = "Thanks for dining!") -> dict:
    """
    Dispatches a physical marketing postcard coupon to a customer address for loyalty rewards.
    """
    if not customer_name or not address:
        raise ValueError("Missing required customer details (customer_name and address are required)")
    if discount_percent <= 0 or discount_percent > 100:
        raise ValueError("Invalid discount percentage (must be between 1 and 100)")

    coupon_code = f"SAVE{int(discount_percent)}"
    logger.info(f"[Post-Pilot] Dispatching postcard to {customer_name} at {address}")
    ...
    return {
        ...
        "message": f"Success: Post-Pilot postcard queued for dispatch to {customer_name}. Code: {coupon_code}. Message: \"{coupon_message}\"."
    }


# ---------------------------------------------------------------------------
# Tool 9: Evaluate Customer Loyalty Milestones
# ---------------------------------------------------------------------------
@mcp.tool()
def evaluate_loyalty_milestone(customer_id: str, name: str, address: str, visit_count: int, total_spend_dollars: float) -> dict:
    """
    Evaluates customer loyalty stats against reward thresholds and queues a marketing postcard if eligible.
    """
    VISIT_MILESTONE = 5
    SPEND_MILESTONE_DOLLARS = 250.0

    if visit_count >= VISIT_MILESTONE or total_spend_dollars >= SPEND_MILESTONE_DOLLARS:
        discount = 20.0 if total_spend_dollars >= SPEND_MILESTONE_DOLLARS else 15.0
        msg = f"Congratulations {name}! Thank you for your continued loyalty. Enjoy {int(discount)}% off your next visit."
        dispatch = send_marketing_postcard(name, address, discount, msg)
        return {
            "eligible": True,
            "milestone_reached": "spend" if total_spend_dollars >= SPEND_MILESTONE_DOLLARS else "visits",
            "dispatch": dispatch
        }
    return {
        "eligible": False,
