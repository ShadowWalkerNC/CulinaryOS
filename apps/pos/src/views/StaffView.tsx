  ...
  return (
    <div className="flex flex-col items-center justify-center h-full bg-[#f8f9fa] animate-fadeIn p-6">
      <div className="bg-white border border-[#e5e7eb] rounded-2xl p-8 max-w-sm w-full shadow-lg text-center space-y-6">
        <div>
          <span className="font-black text-sm tracking-tight text-[#ff5f1f] uppercase block">SquareOS POS Terminal</span>
        ...
        <div className="grid grid-cols-3 gap-3">
          {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map((num) => (
            <button key={num} onClick={() => handleKeyPress(num)}
              className="bg-[#f9fafb] border border-[#e5e7eb] hover:bg-[#f3f4f6] text-[#1f2937] text-lg font-bold py-3.5 rounded-xl transition-colors">
              {num}
            </button>
          ))}
          <button onClick={handleClear}
            className="bg-[#f9fafb] border border-[#e5e7eb] hover:bg-[#f3f4f6] text-red-500 text-xs font-black rounded-xl uppercase transition-colors">
            Clear
          </button>
          ...
          <button onClick={handleLogin} disabled={pin.length < 4}
            className="bg-[#ff5f1f] hover:bg-[#e04f1a] disabled:opacity-40 text-white text-xs font-black rounded-xl uppercase tracking-wider transition-colors shadow-sm">
            OK
          </button>
        </div>
      </div>
